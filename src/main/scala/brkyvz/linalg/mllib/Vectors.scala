 ...
 */
sealed trait Vector extends Serializable with VectorLike {
...
abstract class LazyVVOp(left: VectorLike, 
                        right: VectorLike, 
                        operation: (Double, Double) => Double) extends LazyVector {

  override def size = math.max(left.size, right.size)
...
class LazyDenseVVOp(
    ...
    into: Option[DenseVector]) extends LazyVVOp(left, right, operation) {
  
  override def compute(): Vector = {
    val values = 
      if (into.isEmpty) {
...
abstract class LazyVOp(parent: VectorLike,
                       operation: Double => Double) extends LazyVector {
  override def size = parent.size

}

class LazyDenseVOp(parent: VectorLike,
                   operation: Double => Double,
                   into: Option[DenseVector]) extends LazyVOp(parent, operation) {
